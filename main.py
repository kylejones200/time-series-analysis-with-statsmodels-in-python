...
def main():
    parser = argparse.ArgumentParser(
        description="Time Series Analysis with statsmodels"
    )
    ...
    if config["analysis"]["run_decomposition"]:
        decomposition = decompose_series(
            df["value"], period=config["model"]["decomposition_period"]
        )
        plot_decomposition(decomposition, output_dir / "time_series_decomposition.png")

    if config["analysis"]["run_acf_pacf"]:
        plot_acf_pacf(
            df["value"], lags=30, output_path=output_dir / "acf_pacf_plots.png"
        )
    ...
    if config["analysis"]["run_holt_winters"]:
        ...
        plot_holt_winters_forecast(
            ...
            hw_forecast,
            mape_hw,
            output_dir / "holt_winters_forecast.png",
        )
