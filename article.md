...
**`statsmodels`** provides a lot of tools
...
`statsmodels` covers univariate and
...
outputs of key metrics. And it works well with `pandas` and `numpy`.

Let's check it out. We'll analyze a simulated time series dataset to
demonstrate some of the key features of `statsmodels.`
...
Use `seasonal_decompose` to split the
...
### Key Takeaways
`statsmodels` is the benchmark I use to
...
[GitHub](https://github.com/kylejones200/time_series/blob/main/Statsmodels.ipynb).
