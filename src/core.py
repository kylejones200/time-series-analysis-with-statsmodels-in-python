...
def split_data(
    df: pd.DataFrame, hold_out_days: int = 30
...
def fit_arima(
    train_data: pd.Series, order: tuple[int, int, int] = (2, 1, 2), freq: str = "D"
):
...
def fit_holt_winters(train_data: pd.Series, seasonal_periods: int = 30):
    ...
    model = ExponentialSmoothing(
        train_data, seasonal="add", seasonal_periods=seasonal_periods
    )
    return model.fit()
...
def plot_residuals_diagnostics(model, output_path: Path):
    ...
    for ax in fig.axes:
        plt.suptitle(
            "ARIMA Model Residual Diagnostics", fontsize=12, y=0.98, color="0.2"
        )
    plt.tight_layout()
    plt.savefig(output_path, dpi=100, bbox_inches="tight")
    plt.close()
