...
def generate_synthetic_data(n_samples: int=200, start_date: str='2023-01-01', frequency: str='D', seed: int=42) -> pd.DataFrame:
...
def test_stationarity(series: pd.Series) -> dict[str, Any]:
    ...
    return {'adf_statistic': result[0], 'p_value': result[1], 'is_stationary': result[1] <= 0.05, 'critical_values': result[4]}
...
def forecast_arima(model, steps: int, index: pd.DatetimeIndex):
    ...
    return {'mean': forecast.predicted_mean, 'conf_int': forecast.conf_int(), 'index': index}
...
def plot_time_series(df: pd.DataFrame, train: pd.DataFrame, hold_out: pd.DataFrame, output_path: Path, plot: bool=False):
...
def plot_arima_forecast(train: pd.DataFrame, hold_out: pd.DataFrame, forecast: dict[str, Any], mape: float, output_path: Path, plot: bool = False):
    ...
    ax.fill_between(forecast['index'], forecast['conf_int'].iloc[:, 0], forecast['conf_int'].iloc[:, 1], color='#D4A574', alpha=0.12, label='95% Confidence Interval')
    ax.set_xlabel('Date')
    ax.set_ylabel('Value')
    ax.legend(loc='best')
    plt.savefig(output_path, dpi=100, bbox_inches='tight')
...
def plot_decomposition(decomposition, output_path: Path, plot: bool=False):
    """Plot time series decomposition."""
    fig = decomposition.plot()
    fig.set_size_inches(10, 8)
    for ax in fig.axes:
        plt.suptitle('Time Series Decomposition: Trend, Seasonal, and Residual Components', fontsize=12, y=0.98, color='0.2')
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(output_path, dpi=100, bbox_inches='tight')
    plt.close()

def plot_acf_pacf(series: pd.Series, lags: int=30, output_path: Path | None=None, plot: bool=False):
    """Plot ACF and PACF."""
    if not plot:
        return
    ...
    for ax in axes:
        plt.suptitle('Autocorrelation and Partial Autocorrelation Functions', fontsize=12, y=0.98, color='0.2')
    plt.tight_layout()
    if output_path:
        plt.savefig(output_path, dpi=100, bbox_inches='tight')
        plt.close()
...
def plot_residuals_diagnostics(model, output_path: Path, plot: bool=False):
